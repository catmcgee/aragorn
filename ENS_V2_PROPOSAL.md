...
whether v2's primitives let us move that identity *onchain, ownable, and permissioned* — and
...
From that point *they* mint/burn the subnames under their name onchain. Resolution walks down and
**the deepest resolver wins (longest-suffix match)** — so a 2LD resolver can answer for descendants
*unless* a deeper node sets its own. This is the onchain analogue of the ENSIP-10 wildcard we lean
on today.
...
This is the key insight: **EAC is a general-purpose, onchain, per-name RBAC system.** Aragorn
...
onchain shape of the same idea.
...
recovery) with onchain finality.
...
- Gateway (`gateway.ts:46-95`) reads the `employees` table by `subname_label`, answers `text`/`addr`,
  ...
  onchain resolver pins the signer (`OffchainResolver.sol:61-67`) so a hosted gateway can't forge.
...
signed-offchain gateway, and the institution's RBAC is invisible onchain. v2 lets us tighten all
four.
...
### #1 — Each Ring OWNS its name's subregistry; departments/employees are onchain subname tokens
...
(`ens-setup.ts:163`). Employees/departments become first-class onchain entities the institution
...
onchain subname issuance — so **departments and any employee identity we're willing to publish move
onchain**. Keep CCIP-Read *only* for records that must stay private/high-cardinality (e.g. an
...
subname's *existence and ownership* become the onchain membership proof, not a Postgres row.
...
RBAC (`auth.ts:17`) onto onchain roles scoped to the institution's name:
...
The Ring's API still enforces day-to-day RBAC, but it *derives* authority from onchain EAC grants.
...
Ring's `counterparties` resource**, granted/revoked onchain (reversible, A3). Reads replace the
...
*hash/attestation* onchain and the credential off-chain, or reference an EAS attestation UID.
...
### Ranking rationale
#1 and #3 are the structural core (sovereign onchain identity; name-as-config) and reinforce each
other. #2 is the highest-ceiling idea (onchain institutional governance) but the riskiest to make
...
4. Demo script: spin up a fresh institution → it registers a name → mints two department tokens →
   ...
   `treasury.<inst>.aragornrings.eth` settles, with the party key read **live from the onchain
   subname** (`flows.ts:82` path). Narrate: *"no addresses anywhere; the counterparty is a name the
   institution provably owns and controls onchain."*
